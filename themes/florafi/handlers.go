...
// Handler processes all front-end HTTP requests for the FloraFi theme.
type Handler struct {
...
func (h *Handler) LoadPageTemplates(t coreTheme.Theme) error {
	...
	h.templates = bundle
	log.Printf("[florafi] Loaded %d page templates", len(bundle))
	return nil
}
...
func (h *Handler) render(c *gin.Context, page string, data interface{}) {
	...
	if err := tmpl.ExecuteTemplate(c.Writer, "base", data); err != nil {
		log.Printf("[florafi] Template render error [%s]: %v", page, err)
	}
}
...
func (h *Handler) Home(c *gin.Context) {
	...
	if err != nil {
		log.Printf("[florafi] Error getting home data: %v", err)
...
func (h *Handler) About(c *gin.Context) {
	...
	if err != nil {
		log.Printf("[florafi] Error getting about data: %v", err)
		c.String(http.StatusInternalServerError, "Internal server error")
		return
	}
...
func (h *Handler) Products(c *gin.Context) {
	...
	if err != nil {
		log.Printf("[florafi] Error getting products data: %v", err)
		c.String(http.StatusInternalServerError, "Internal server error")
		return
	}
...
func (h *Handler) Services(c *gin.Context) {
	...
	if err != nil {
		log.Printf("[florafi] Error getting services data: %v", err)
		c.String(http.StatusInternalServerError, "Internal server error")
		return
	}
...
func (h *Handler) Showcase(c *gin.Context) {
	...
	if err != nil {
		log.Printf("[florafi] Error getting showcase data: %v", err)
		c.String(http.StatusInternalServerError, "Internal server error")
		return
	}
...
func (h *Handler) Blog(c *gin.Context) {
	...
	if err != nil {
		log.Printf("[florafi] Error getting blog data: %v", err)
		c.String(http.StatusInternalServerError, "Internal server error")
		return
	}
...
func (h *Handler) Contact(c *gin.Context) {
	...
	if err != nil {
		log.Printf("[florafi] Error getting contact data: %v", err)
		c.String(http.StatusInternalServerError, "Internal server error")
		return
	}
...
func (h *Handler) ContactSubmit(c *gin.Context) {
	...
	if err := h.pageService.SubmitContact(c, name, email, phone, message); err != nil {
		log.Printf("[florafi] Error saving contact message: %v", err)
		data, _ := h.pageService.GetContactData()
...
func (h *Handler) ShowcaseDetail(c *gin.Context) {
	...
	if err != nil || data == nil {
		c.String(http.StatusNotFound, "Use case not found")
		return
	}
