...
- **commerce-usdt** — USDT（ERC-20）加密货币卫星网关：为每个订单派生唯一 HD 收款地址，验证以太坊 RPC 身份与日志，原子推进扫链游标，按链上时间判定到期并幂等结算。watch-only xpub，服务器无花费权限；实现可扩展到其他 EVM 链，当前仅支持以太坊。只依赖 `core/commerce` 契约。
