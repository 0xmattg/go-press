...
# 一次性把 gopress CLI 安装到 $GOBIN（或 $GOPATH/bin）
make install
# （不想全局安装可以执行 `make gopress`，产物在 ./build/gopress）
...
gopress build -o ./myserver # 自定义输出路径
```

`make help` 列出所有 Make 目标，`gopress help` 列出所有 CLI 子命令。

> **1c1g 小机器编译？** `go build` 默认按 CPU 并行编译，容易被小 VPS 的 OOM killer 杀掉。前面加 `GOFLAGS="-p=1 -v"` 强制串行即可，例如 `GOFLAGS="-p=1 -v" gopress build`。详见[安装指南](docs/guide/zh-CN/getting-started/installation.md#小内存机器编译)。
