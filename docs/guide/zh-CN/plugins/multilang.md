...
自定义 `PageService` / 自定义 `SEOMeta` 的主题需要显式接入当前请求语言：在 `ForRequest(c)` 中保存 `gin.Context`，并在构建 SEO 后调用 `coreTheme.ApplySiteOptionOverridesFromOptionsForRequest(c, options, i18nMgr, seoBuilder, &seo)`。详细规范见 [主题 SEO 接入规范](../themes/seo-integration.md)。
