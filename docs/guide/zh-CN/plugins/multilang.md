...
| `PageService.ReqCtx` | 主题 `ForRequest(c)` 把 ctx 存到克隆服务上（`ReqCtx` 字段继承自 `coreTheme.BasePageService`），详情页 `Get*Detail` 用 `s.Content.FindBySlugScoped(s.ReqCtx, ...)` —— **修复了"主题用自己的 contentRepo 绕过 scope"的细节坑** |
...
| **网站设置翻译** | 后台「翻译管理 → 网站设置翻译」可翻译系统 `site_name` 和 `site_description`。前台 SEO 标题、Meta 描述、OG 描述和模板 `.Settings` 会按当前语言读取这些值 |
...
| `gp_plgn_multilang_string_translations` | 字符串翻译（domain: `"theme"` = UI 字符串, `"option"` = 主题设置和系统网站设置翻译；name/language_code/value） |
...
BaseTheme 路径自动支持网站设置翻译；主题模板继续使用 `{{pageTitleFor . $fallbackTitle}}` 和 `{{seoHeadFor .}}` 即可。

嵌入 `coreTheme.SEOPageService` 的主题无需手动处理：`BuildArchiveSEO` / `BuildContentSEO` 已按当前请求语言调用 `ApplySiteOptionOverridesFromOptionsForRequest`。只有**完全自建 `SEOMeta`**（不走 `SEOPageService`）的主题，才需要在 `ForRequest(c)` 中保存 `gin.Context`、并在构建 SEO 后自己调 `coreTheme.ApplySiteOptionOverridesFromOptionsForRequest(c, options, i18nMgr, seoBuilder, &seo)`。详细规范见 [主题 SEO 接入规范](../themes/seo-integration.md)。
