...
因此 Google OIDC 与未来的钱包登录可以并存，core 不需要加入 Provider 名称或协议特判。
...
MetaMask 应作为另一个身份插件实现，并优先使用成熟的 Sign-In with Ethereum（SIWE / EIP-4361）库。建议流程：

1. 插件创建绑定站点域名、短期有效且只能使用一次的 nonce；
2. 浏览器钱包签署结构化 SIWE message；
3. 插件验证 domain、URI、nonce、签发/过期时间、chain ID 与恢复出的地址；
4. 插件将已验证主体转换为 `VerifiedIdentity`；
5. Core 继续复用与 OIDC 相同的注册、关联、角色和 Session 策略。

不能直接信任 JSON 中上传的钱包地址。钱包插件可以拥有 challenge 表和链相关 metadata，但不能绕过 `IdentityBroker` / `PublicAuth` 自行创建用户或 Session。
