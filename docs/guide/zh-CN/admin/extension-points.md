...
插件可通过 Settings Provider 接口提供独立后台设置页。Admin 负责统一路由、
布局、当前后台语言、认证与 `plugin.read` / `plugin.update` 权限；插件负责
设置模板、当前数据和保存逻辑。插件卡片只有在实现对应 Provider 时才显示
「插件设置」入口。

设置模板不得硬编码中文或英文，应使用后台翻译 helper 或组件自己的 locale
文件。如果组件只提供一种语言，后台应回退到可用语言，而不是隐藏设置 UI。
...
- 插件启停后 core 会重建 Router 并清理相关缓存，使中间件、路由、后台 UI
  和前台输出同步反映当前状态
